...
// The ResultLogFunc type is an adapter to allow the use of ordinary
// function as ResultLog mutator.
type ResultLogFunc func(context.Context, *ent.ResultLogMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ResultLogFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ResultLogMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ResultLogMutation", m)
	}
	return f(ctx, mv)
}
