...
func (cq *CommandQuery) QueryLogs() *ResultLogQuery {
	query := &ResultLogQuery{config: cq.config}
...
func (cq *CommandQuery) WithLogs(opts ...func(*ResultLogQuery)) *CommandQuery {
	query := &ResultLogQuery{config: cq.config}
...
func (cq *CommandQuery) sqlAll(ctx context.Context) ([]*Command, error) {
	...
	if query := cq.withLogs; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, cq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "logs": %w`, err)
		}
		query.Where(resultlog.IDIn(edgeids...))
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "logs" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Logs = append(nodes[i].Edges.Logs, n)
			}
		}
	}
